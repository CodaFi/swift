...
//
//  - `StaticArray<Element>` is a fast, contiguous array of `Element` with
...
///
/// The `StaticArray` type is a specialized array that always stores its
...
/// APIs, using `StaticArray` may be more efficient and have more
/// predictable performance than `Array`. If the array's `Element` type is a
/// struct or enumeration, `Array` and `StaticArray` should have similar
/// efficiency.
///
/// For more information about using arrays, see `Array` and `ArraySlice`, with
/// which `StaticArray` shares most properties and methods.
@frozen
public struct StaticArray<Element, let N: UInt>: _DestructorSafeContainer {
  @usableFromInline
  internal typealias _Buffer = UnsafeMutablePointer<Element>
...
extension StaticArray {
  ...
  internal func _checkIndex(_ index: Int) {
    _precondition(index <= endIndex, "StaticArray index is out of range")
    _precondition(index >= startIndex, "Negative StaticArray index is out of range")
  }
...
extension StaticArray: _ArrayProtocol {
...
extension StaticArray: RandomAccessCollection, MutableCollection {
  ...
  public typealias Iterator = IndexingIterator<StaticArray>

  /// The position of the first element in a nonempty array.
  ///
  /// For an instance of `StaticArray`, `startIndex` is always zero. If the array
...
extension StaticArray: ExpressibleByArrayLiteral {
  ...
  /// strings:
  ///
  ///     let ingredients: StaticArray =
  ///           ["cocoa beans", "sugar", "cocoa butter", "salt"]
  ///
  /// - Parameter elements: A variadic list of elements of the new array.
  @inlinable
  public init(arrayLiteral elements: Element...) {
    self.init(_buffer: StaticArray(elements)._buffer)
  }
}


extension StaticArray: RangeReplaceableCollection {
  ...
  public init() {
    ...
    let raw = Builtin.allocRawStack(N._builtinWordValue, align)
...
extension StaticArray: CustomReflectable {
...
extension StaticArray: CustomStringConvertible, CustomDebugStringConvertible {
  ...
  public var debugDescription: String {
    return _makeCollectionDescription(withTypeName: "StaticArray")
  }
}
...
extension StaticArray {
  ...
  internal func _cPointerArgs() -> (AnyObject?, UnsafeRawPointer?) {
    ...
    let n = StaticArray(self._buffer)._buffer
...
extension StaticArray {
  ...
  public func withUnsafeBufferPointer<R>(
    ...
    return try body(UnsafeBufferPointer(start: _buffer, count: N))
  }
  ...
  public mutating func withUnsafeMutableBufferPointer<R>(
    ...
    var work = StaticArray<Element, N>()
    (work, self) = (self, work)
    ...
    defer {
      _precondition(
        ...
        "StaticArray withUnsafeMutableBufferPointer: replacing the buffer is not allowed")
...
extension StaticArray {
  ...
  public mutating func replaceSubrange<C>(
    ...
    _precondition(subrange.lowerBound >= self._buffer.startIndex,
      "StaticArray replace: subrange start is negative")

    _precondition(subrange.upperBound <= _buffer.endIndex,
      "StaticArray replace: subrange extends past the end")

    var idx = subrange.start
...
extension StaticArray: Equatable where Element: Equatable {
  ...
  @inlinable
  public static func ==(lhs: StaticArray<Element, N>, rhs: StaticArray<Element, N>) -> Bool {
