  ...
  private:
    ...
    CommandSet subsequentJobsNeeded(const Job *FinishedCmd,
      ...
      if (!Comp.getIncrementalBuildEnabled() &&
          !Comp.getEnableCrossModuleIncrementalBuild())
        return {};
  ...
  private:
    ...
    CommandSet computeFirstRoundCompileJobsForIncrementalCompilation() {
      ...
      if (Comp.getEnableCrossModuleIncrementalBuild()) {
        bool success = readDriverDependencyGraph(Comp.getExternalSwiftDepsFilePath(),
                                                 getFineGrainedDepGraph());
        if (!success) {
          return getEveryCompileJob(); // Load error, just run them all
        }
//        llvm::outs() << "----" << "PRIORS" << "----" << "\n";
//        getFineGrainedDepGraph().forEachNode([](const auto *node){
//          node->dump();
//        });
//        llvm::outs() << "----" << "\n";
      }
...
Compilation::performJobsImpl(std::unique_ptr<TaskQueue> &&TQ) {
  ...
  if (!CompilationRecordPath.empty()) {
    ...
    if (EnableCrossModuleIncrementalBuild) {
      for (const auto &external : result.depGraph.getIncrementalExternalDependencies()) {
        const auto *externalJob = addExternalJob(
            std::make_unique<Job>(getDerivedOutputFileMap(), external));
        auto buffer = llvm::MemoryBuffer::getFile(external);
        if (!buffer) {
          continue;
        }
        result.depGraph.loadFromSwiftModuleBuffer(
                externalJob, *buffer.get(), getDiags());
      }
      // Write out our priors adjacent to the build record so we can pick
      // them up in a subsequent build.
      writeDriverDependencyGraphToPath(getDiags(),
                                       getExternalSwiftDepsFilePath(),
                                       result.depGraph);
    }
    return result;
  } else {
