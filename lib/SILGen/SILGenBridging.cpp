...
void SILGenFunction::emitTestThunk(SILDeclRef thunk) {
  assert(thunk.isTestThunk);
  SILDeclRef native = thunk.asTestThunk(false);
  ...
  // Now, enter a cleanup used for casting the arguments.
  Scope argScope(Cleanups, CleanupLocation::get(loc));

  // Force-cast the argument.
